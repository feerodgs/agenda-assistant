...
@Component
public class AmanhaHandler implements CommandHandler {

    private static final Logger log = LoggerFactory.getLogger(AmanhaHandler.class);

    private final UserConnectionRepository userConnectionRepository;
    private final GoogleCalendarService calendarService;

    public AmanhaHandler(UserConnectionRepository userConnectionRepository,
    ...
    public List<BotResponse> handle(long chatId, Update update) {
        ...
        try {
            ...
            List<Event> eventos = calendarService.listarEventosDoDia(conn, amanha);
            String cabecalho = "Agenda de amanha (" + amanha.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) + "):";
            return BotResponse.text(calendarService.formatarListaEventos(eventos, cabecalho)).asList();
        } catch (Exception e) {
            log.error("[AmanhaHandler] Erro ao buscar eventos — chatId={}", chatId, e);
